...
# 1. SYNCHRONIZED ALPHABETICAL CLASSES
CLASS_NAMES = [
...
try:
    # Load model with compilation flags completely disabled to prevent graph freezes
...
# 2. IMAGE PIPELINE - UNCHANGED RAW NUMERIC MATRIX PASSING
...
def predict():
    ...
    try:
        ...
        # --- THE WORKAROUND FOR DATA AUGMENTATION LAYERS ---
        # Explicitly pass training=False to tell the graph to disable augmentation steps 
        # and process the raw input matrix directly.
        predictions = model(processed_tensor, training=False).numpy()
        
        # Unpack predictions array
